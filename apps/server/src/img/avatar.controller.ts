...
import { isWeixinImageUrl } from '@server/common/url-security';
...
const cache = new Map<string, { buf: Buffer; type: string; ts: number }>();
...
export class AvatarController {
  ...
  async getAvatar(@Param('feedId') feedId: string, @Res() res: Response) {
    ...
    if (!isWeixinImageUrl(url)) return res.status(403).end();
